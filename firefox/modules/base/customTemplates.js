function CustomTemplateWatcher() {
	...
	function createSelect(templates, useTemplates) {
		...
		(async () => {
			const storage = await browser.storage.sync.get(["useTemplates"]);
		...
		btn.addEventListener("click", async () => {
			const storage = await browser.storage.sync.get(["templates"]);
	...
	async function update(reason) {
		const storage = await browser.storage.sync.get(["useCustomTemplates", "customTemplates", "useTemplates"]);
